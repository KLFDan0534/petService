  ...
  return mount(UserLayout, {
    global: {
      ...
      stubs: {
        ...
        ThemeToggle: true,
        AppIcon: { template: '<span class="el-icon-stub"><slot /></span>' },
...
describe('UserLayout 导航样式规则(仿 proxyAI 胶囊/玻璃)', () => {
  let css
  beforeAll(() => {
    const file = resolve(__dirname, '../components/layout/UserLayout.vue')
    css = readFileSync(file, 'utf8').split('<style scoped>')[1].split('</style>')[0]
  })

  it('默认(顶部)导航栏: 全宽紧贴顶部, 背景透明(与页面 canvas 背景一致)', () => {
    expect(css).toMatch(/:global\(header\.user-header\) \{[\s\S]*?top: 0;/)
    expect(css).toMatch(/:global\(header\.user-header\) \{[^}]*?background: transparent;/)
    expect(css).not.toMatch(/:global\(header\.user-header\) \{[^}]*?background: var\(--color-card\);/)
  })
...
expect(css).toMatch(/user-header-scrolled \.user-header-shell\) \{[\s\S]*?box-shadow:/)
  ...
  it('滚动阈值使用 proxyAI 的 80px(scrollY > 80)', () => {
    const script = readFileSync(resolve(__dirname, '../components/layout/UserLayout.vue'), 'utf8')
