...
public interface AiChatService {
     ...
     * <p>调用链：Controller层接收请求 → AiChatService.chat() → 组装为多轮消息列表 → chat(List) → OpenAiChatModel.chat() → DeepSeek API</p>
     ...
     * <p>调用链：Controller层接收请求 → AiChatService.chat() → 转换为langchain4j.ChatMessage → 构建ChatRequest → OpenAiChatModel.chat() → DeepSeek API</p>
